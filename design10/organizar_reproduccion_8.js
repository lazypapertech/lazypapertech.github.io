...
function renderFrames(previewOnly = false) {
  ...
  // 2️⃣ Renderizar frame del cache
  if (frameIndex !== ultimoFrameDibujado) {
    ultimoFrameDibujado = frameIndex;
    ...
    if (!precargaCompleta) {
      ctx_principal.clearRect(0, 0, rect.width, rect.height);
      draw_text("Preparing video...", rect);
      return; // ✅ Salir temprano si no está listo
    }

    // ✅ Obtener frames del cache
    if (cache_segundos.has(segundo)) {
      ...
      if (frames && frames[frameEnSegundo]) {
        ...
        ctx_principal.clearRect(0, 0, rect.width, rect.height);
        draw_text("Frame missing...", rect);
        console.error(`Frame ${frameEnSegundo} no existe en segundo ${segundo}`);
      }
    } else {
      ctx_principal.clearRect(0, 0, rect.width, rect.height);
      draw_text("Second not cached...", rect);
      console.error(`Segundo ${segundo} no está en cache`);
    }
  }
    ...
    linea.forEach(item => {
      ...
      if (activo) {
        ...
        if (!videoDibujado && item.media.tagName === 'VIDEO' && 
          ...
          // Solo usar video local si la precarga no está completa
          if (!precargaCompleta || !cache_segundos.has(segundo)) {
            const rect = canvas_principal.getBoundingClientRect();
            canvas_principal.width = rect.width;
            canvas_principal.height = rect.height;
            ctx_principal.drawImage(item.media, 0, 0, rect.width, rect.height);
...
playBtn.onclick = async () => {  
  ...
  // ✅ PRECARGAR SIEMPRE antes de reproducir
  console.log('⏳ Precargando trozos disponibles...');
  
  const rect = canvas_principal.getBoundingClientRect();
  canvas_principal.width = rect.width;
  canvas_principal.height = rect.height;
  ctx_principal.clearRect(0, 0, rect.width, rect.height);
  draw_text("Preparing video, please wait...", rect);
  
  await precargarTodosLosTrozos();
...
const cache_segundos = new Map(); // Map<segundo, ImageBitmap[]>
...
async function precargarTodosLosTrozos() {
  console.log("🔄 INICIO: precargarTodosLosTrozos()");
  console.log("trozos_guardados.length:", trozos_guardados.length);
  console.log("trozos_guardados:", trozos_guardados);
  
  precargaCompleta = false;
  
  // Verificar si hay trozos
  if (trozos_guardados.length === 0) {
    console.error("❌ No hay trozos guardados!");
    precargaCompleta = true; // Marcar como completa para no bloquear
    return;
  }
  
  for (let i = 0; i < trozos_guardados.length; i++) {
    console.log(`Revisando índice ${i}:`, trozos_guardados[i] ? "EXISTE" : "VACÍO");
    
    if (trozos_guardados[i]) {
      console.log(`⏳ Decodificando trozo ${i}...`);
      
      try {
        const frames = await TrozoDecoder.decodificarTrozo(i);
        
        if (frames) {
          cache_segundos.set(i, frames);
          console.log(`✅ Trozo ${i} cacheado (${frames.length} frames) - Progreso: ${i + 1}/${trozos_guardados.length}`);
        } else {
          console.error(`❌ Trozo ${i} devolvió null`);
        }
      } catch (error) {
        console.error(`❌ Error decodificando trozo ${i}:`, error);
      }
    }
  }
  
  precargaCompleta = true;
  console.log("✅ COMPLETADO: Todos los trozos precargados");
  console.log("cache_segundos.size:", cache_segundos.size);
}
