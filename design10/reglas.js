...
function crearPseudoRegla() {
    ...
    for (let i = 0; i <= TOTAL_PUNTOS; i++) {
        const x = PADDING + i * SEGMENTO*(2.0/CALIBRACION);

        // Guardamos todos los marcadores
        todosLosMarcadores.push({ x: x, top: 0 });

	const puntos_segundo = 5 * 0.5 * CALIBRACION;
        // Labels cada 5 puntos
        if (i % puntos_segundo === 0) {
            todosLosLabels.push({
                x: x,
                top: alturaPseudo / 3 - 4, // centro de la bolita
                texto: formatearTiempoDecimal_2(i / puntos_segundo)
            });
        }
    }
}
...
/*
//no es compatible con calibracion = 4
...
*/
function timeToX_00(timeString, paso = 0.2) {
  const parts = timeString.split('.');
  const timePart = parts[0]; // HH:MM:SS
  ...
  const totalSecondsRaw =
    hours * 3600 + minutes * 60 + seconds + parseInt(decimalPart) / 10;

  // 🔹 ajustar al múltiplo del paso (0.2, 0.5, 0.1, etc.)
  const totalSeconds =
    Math.round(totalSecondsRaw / paso) * paso;

  // relación base
  const unidadesPorPaso = SEGMENTO; // 40 unidades de x por cada "paso"
  const x = PADDING + (totalSeconds / paso) * unidadesPorPaso;
...
function timeToX(timeString, pasoRedondeo = 0.05) {
  pasoRedondeo = 0.4/CALIBRACION;
  const parts = timeString.split('.');
  const timePart = parts[0]; // HH:MM:SS
  const decimalPart = parts[1] || '0';
  const [hours, minutes, seconds] = timePart.split(':').map(Number);
  ...
  const totalSecondsRaw =
    ...
    parseInt(decimalPart) / Math.pow(10, decimalPart.length); // ← fix

  const totalSeconds = redondearAPaso(totalSecondsRaw, pasoRedondeo);

  const x = PADDING + (totalSeconds / 0.2) * SEGMENTO; 
  return x;
} 
...
function crearRegla() { 
    ...
    let iteraciones_extra=0;
    let index_item = -1;
    if (selected_rect && current_view == "group"){  
       index_item = selected_rect.dataset.index_item; 
       extra_rows = dic2[selected_rect.dataset.filetype].length;
       iteraciones_extra = extra_rows-1;
       iteraciones_extra = iteraciones_extra - 2; //ai_filter, ai_mask
       console.log("error regla:",selected_rect.dataset.filetype,extra_rows);
    }
    console.log("view_filter error:",current_view);
    if (selected_rect && current_view == "view_filter"){  
       index_item = selected_rect.dataset.index_item;  
       iteraciones_extra = 0;  
    }
    ...
    for (let i = 0; i < data.length; i++) { 
        const sublist = data[i];
        console.log("sublist[0].index_item:",sublist[0].index_item,index_item);

        if (!sublist || sublist.length === 0) continue;  
        if (iteraciones_extra==0){
		if (sublist[0].property !== "filename") continue;
	}else{ 
		if (sublist[0].property == "filename") continue; 
	}
		...
		if (index_item!=sublist[0].index_item) continue;
	} 
 
         
         
	if (sublist[0].filetype == "audio") continue; 
    ...
    */
    ...
    if(current_view == "group" || current_view == "view_filter" || current_view == "view_mask"){
	if (index_item!=sublist[0].index_item) continue;
    }else{
        const propiedades_visibles = dic2[sublist[0].filetype];
        //console.log(current_view," propiedades_visibles:",sublist[0].property);
	if (!propiedades_visibles.includes(current_view)){
		continue;
	}
    } 

    // Ejecutar iteraciones_extra + 1 veces
    const vecesAEjecutar = Math.max(1, iteraciones_extra + 1);
    
    for (let iter = 0; iter < vecesAEjecutar; iter++) {
        const top = Math.floor(offsetY + rowIndex * ALTURA_REGLA);
...
return;
    let lineas_totales = rectangulos.length;

    const dic2 = {
        "image": ["position", "scale", "rotation", "opacity","ratio"],
        "video": ["position", "scale", "rotation"],
	"audio": [],
	"text": ["position", "scale", "rotation", "opacity","item_reveal"]
    };
    let extra_rows=0;
    if (selected_rect){  
       extra_rows = dic2[selected_rect.dataset.filetype].length;
       lineas_totales = extra_rows;
       console.log("error regla:",selected_rect.dataset.filetype,extra_rows);
    }
    ...
    for (let i = 0; i < lineas_totales; i++) {//rectangulos.length
        const top = Math.floor(offsetY + i * (ALTURA_REGLA));
        const fondo = document.createElement('div');
...
function arreglar_posicion(distancia){  
console.log("DISTANCIA:",distancia);
    if (distancia <= 40) {   
    	llevarRectanguloAbajo();	
    	console.log("ABAJO");
    } 
}
