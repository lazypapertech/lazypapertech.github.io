...
console.log("src[0]:", srcGroup[0]);
console.log("dst[0]:", dstGroup[0]);
console.log("bg_id src:", srcGroup[0].bg_id, typeof srcGroup[0].bg_id);
console.log("bg_id dst:", dstGroup[0].bg_id, typeof dstGroup[0].bg_id);
  // ---- recorremos solo esa fila ----
  for (let i = 0; i < srcGroup.length; i++) {

    const src = srcGroup[i];
    console.log("src:", JSON.stringify(srcGroup[i]));
    ...
    console.log("dst:", JSON.stringify(dstGroup[i]));
...
function update_rectangles_2() {
console.log("unica_regla.rectangulos view:", JSON.stringify(unica_regla.rectangulos));
console.log("list rectangulos view 1:", JSON.stringify(rectangulos));

  const BASE_X = 40;
  const PX_PER_SEC = 200;

  if (!Array.isArray(unica_regla?.rectangulos)) return;

  // Recorremos TODAS las sublistas de rectangulos (destino)
  for (let index_row = 0; index_row < rectangulos.length; index_row++) {
    const dstGroup = rectangulos[index_row];
    if (!Array.isArray(dstGroup) || dstGroup.length === 0) continue;

    for (const dst of dstGroup) {
      if (!dst) continue;

      // Obtenemos la sublista fuente correcta via index_global_row
      const srcGroup = unica_regla.rectangulos[dst.index_global_row];
      if (!Array.isArray(srcGroup)) continue;

      // Buscamos el elemento fuente por bg_id
      const src = srcGroup.find(s => s.bg_id === dst.bg_id);
      if (!src) continue;

      const tStart = timeToSeconds(src.start);
      const tEnd   = timeToSeconds(src.end);
      dst.posicion = BASE_X + PX_PER_SEC * tStart;
      dst.ancho    = PX_PER_SEC * (tEnd - tStart);
  ...
  console.log("list rectangulos view 2:", JSON.stringify(rectangulos));
}

function update_rectangles_4() {
  const BASE_X = 40;
  const PX_PER_SEC = 200;
  ...
  for (let index_row = 0; index_row < rectangulos.length; index_row++) {
    ...
    const ref = dstGroup.find(d => d?.elemento?.dataset?.index_global_row != null);
    if (!ref) continue;

    const index_global_row = ref.elemento.dataset.index_global_row;
    const srcGroup = unica_regla?.rectangulos?.[index_global_row];
    if (!Array.isArray(srcGroup)) continue;
    ...
    for (let i = 0; i < srcGroup.length; i++) {
      ...
      // Busca el dst que tiene el mismo índice real dentro de dstGroup
      const dst = dstGroup.find(d => 
        d?.elemento?.dataset?.index_col == i  // ← índice dentro del grupo
      ) ?? dstGroup[i];                        // fallback si no hay dataset
      ...
      const tEnd   = timeToSeconds(src.end);
      dst.posicion = BASE_X + PX_PER_SEC * tStart;
      dst.ancho    = BASE_X + PX_PER_SEC * tEnd - dst.posicion;
