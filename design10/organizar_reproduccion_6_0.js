...
const TARGET_FPS = 5; // Ajusta según tu video (24, 25, 30, etc.)
...
function retroceder_suavemente(velocidadMultiplicador = 1) { 
  ...
  // Retornar una promesa que se resuelve cuando termine el retroceso
  return new Promise((resolve) => {
    ...
    function loopRetroceso(timestamp) {
      if (!retrocediendoActivo) {
        retrocediendoAnimationId = null;
        resolve(); // Resolver cuando se cancela externamente
        return;
      }
      ...
      if (globalTime - retroceso <= 0) {
        ...
        actualizarScroll();
        detener_retroceso();
        resolve(); // Resolver cuando llega a 0
        return;
      }
