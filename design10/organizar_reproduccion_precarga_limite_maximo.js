...
function cancelar_decodificaciones(){
...
playBtn.onclick = async () => {  
  ...
  // ✅ FORZAR DIRECCIÓN ADELANTE AL PRESIONAR PLAY
  reproductorTrozos.forzarDireccionAdelante();
...
/* ===========================
   LIMITE MAXIMO DE DECODIFICACIONES SIMULTANEAS
=========================== */
...
async function precargarTresSegundos(tiempoInicio) {
  const segundoActual = Math.floor(tiempoInicio);
  const segundoSiguiente = segundoActual + 1;
  const segundoTercero = segundoActual + 2;
  ...
  if (trozos_guardados[segundoTercero]) {
    for (let subIndex = 0; subIndex < 5; subIndex++) {
      const frameInicio = subIndex * reproductorTrozos.FRAMES_POR_SUBTROZO;
      const subKey = reproductorTrozos.getSubTrozoKey(segundoTercero, frameInicio);
      
      if (!reproductorTrozos.subTrozos.has(subKey)) {
        console.log(`📥 Precargando ${segundoTercero}-${subIndex}`);
        promesas.push(reproductorTrozos.cargarSubTrozo(segundoTercero, frameInicio));
      }
    }
  }
...
class ReproductorTrozos {
  constructor(canvas) {
    ...
    this.MAX_TROZOS_EN_COLA = 5; // ✅ Máximo 6 trozos decodificándose simultáneamente
    this.colaTrozos = new Map(); // ✅ Map para rastrear orden: { segundo: timestamp }
    this.contadorOrden = 0; // ✅ Contador incremental para ordenar
  }
  ...
  cancelarTodasDecodificaciones() {
    ...
    this.colaTrozos.clear(); // ✅ Limpiar cola
    this.contadorOrden = 0; // ✅ Resetear contador
  ...
  // ✅ NUEVA FUNCIÓN: Gestionar cola de trozos
  gestionarColaTrozos(segundoNuevo) {
    // Si el trozo ya está en cola, no hacer nada
    if (this.colaTrozos.has(segundoNuevo)) {
      return;
    }
    
    // Si hay espacio, simplemente agregar
    if (this.colaTrozos.size < this.MAX_TROZOS_EN_COLA) {
      this.colaTrozos.set(segundoNuevo, this.contadorOrden++);
      console.log(`📋 Trozo ${segundoNuevo} agregado a cola (${this.colaTrozos.size}/${this.MAX_TROZOS_EN_COLA})`);
      return;
    }else{
    ...
    // Si está llena, cancelar el más antiguo
    let segundoMasAntiguo = null;
    let ordenMasAntiguo = Infinity;
    
    this.colaTrozos.forEach((orden, segundo) => {
      if (orden < ordenMasAntiguo) {
        ordenMasAntiguo = orden;
        segundoMasAntiguo = segundo;
      }
    });
    
    if (segundoMasAntiguo !== null) {
      console.log(`🚫 Cola llena (${this.colaTrozos.size}/${this.MAX_TROZOS_EN_COLA}), cancelando trozo ${segundoMasAntiguo}`);
      this.cancelarTrozo(segundoMasAntiguo);
      this.colaTrozos.delete(segundoMasAntiguo);
    }
  ...
  // ✅ NUEVA FUNCIÓN: Cancelar todos los sub-trozos de un segundo específico
  cancelarTrozo(segundo) {
    console.log(`🛑 Cancelando trozo completo: ${segundo}`);
    
    // Buscar todos los sub-trozos de este segundo
    const subTrozosACancelar = [];
    this.subTrozos.forEach((data, key) => {
      const [seg] = key.split('-').map(Number);
      if (seg === segundo) {
        subTrozosACancelar.push(key);
      }
    });
    
    // Cancelar cada sub-trozo
    subTrozosACancelar.forEach(subKey => {
      const data = this.subTrozos.get(subKey);
      ...
      if (data) {
        ...
        this.subTrozos.delete(subKey);
        console.log(`   ✓ Sub-trozo ${subKey} cancelado`);
      }
    });
    
    console.log(`✅ Trozo ${segundo} completamente cancelado (${subTrozosACancelar.length} sub-trozos)`);
  }
  
  // ✅ NUEVA FUNCIÓN: Eliminar trozo de la cola cuando termina
  marcarTrozoCompletado(segundo) {
    if (this.colaTrozos.has(segundo)) {
      this.colaTrozos.delete(segundo);
      console.log(`✅ Trozo ${segundo} completado y removido de cola (${this.colaTrozos.size}/${this.MAX_TROZOS_EN_COLA})`);
    }
  }
  ...
  // ✅ FUNCIÓN MODIFICADA: Precargar según dirección con gestión de cola
  precargarSiguientesSegundos(segundoActual) {
    ...
    if (segundo1 >= 0 && trozos_guardados[segundo1]) {
      // ✅ Gestionar cola antes de cargar
      this.gestionarColaTrozos(segundo1);
    ...
    if (segundo2 >= 0 && trozos_guardados[segundo2]) {
      // ✅ Gestionar cola antes de cargar
      this.gestionarColaTrozos(segundo2);
  ...
  async cargarSubTrozo(segundo, frameInicial, intento = 1) {
    ...
    this.subTrozos.set(subKey, { frames: [], cargando: true, segundo }); // ✅ Guardar segundo
    const data = this.subTrozos.get(subKey);
      ...
      const decoder = new VideoDecoder({
        ...
        error: (e) => {  
          ...
          if (intento < this.MAX_REINTENTOS) {
            ...
            // ✅ Si falla definitivamente, marcar trozo como completado en cola
            this.marcarTrozoCompletado(segundo);
          }
      ...
      try {
        ...
        decoder.flush()
          .then(async () => {
            ...
            // ✅ Si este es el último sub-trozo del segundo, marcarlo como completado
            if (subIndex === 4) { // Último sub-trozo (0-4)
              this.marcarTrozoCompletado(segundo);
            }
          ...
          .catch(err => {  
            ...
            // ✅ Marcar como completado si falla
            this.marcarTrozoCompletado(segundo);
        ...
        if (!decoderCerrado) {
          try {
            decoder.close();
            decoderCerrado = true;
            this.decodersActivos.delete(decoder);
          } catch (e) {}
        }
