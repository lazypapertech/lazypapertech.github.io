...
tabButtonsSettings.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (visiblePanel) {
      // Ocultar todos los contenidos del panel visible
      const tabContentsSettings = visiblePanel.querySelectorAll('.tab-content-general-settings, .tab-content-advanced-settings');
      tabContentsSettings.forEach(c => c.classList.remove('active'));
      
      // Mostrar el contenido correspondiente usando la clase
      const targetClass = btn.dataset.tab.replace('general-tab-content-settings', 'general-tab-content-settings')
                                          .replace('advanced-tab-content-settings', 'advanced-tab-content-settings');
      const activeTab = visiblePanel.querySelector(`.${targetClass}`);
      if (activeTab) {
